...
export default function ProductDetailPage({ params }: ProductPageProps) {
    ...
    if (!product) {
        return (
            <div className="min-h-screen pt-24 flex items-center justify-center bg-aqua-50/30">
                <div className="text-center">
                    <h1 className="text-2xl font-bold text-deep-blue-900 mb-4 animate-float">Product Not Found</h1>
                    <Link href="/products" className="btn btn-primary btn-ripple shadow-glow-aqua">
                        Browse All Products
                    </Link>
                </div>
            </div>
    ...
    return (
        <MobileGestureLayout onPullDown={handlePullDown} key={slug}>
            <div className="min-h-screen bg-white pt-16 pb-32">
                {/* Compact Header */}
                <div className="bg-white/95 backdrop-blur-md sticky top-0 z-40 border-b border-steel-100 flex items-center px-4 h-14 shadow-sm">
                    <Link href={backLink} className="flex items-center gap-2 text-steel-500 hover:text-aqua-600">
                ...
                <div className="px-0 md:container-custom md:px-4 md:py-6">
                    <div className={`grid gap-0 md:gap-8 ${hasImages ? 'lg:grid-cols-2' : ''}`}>
                        {/* Product Image - Full Width on Mobile, Compact */}
                        {hasImages && (
                                ...
                                <div className="relative w-full h-full">
                                    ...
                                    {images.length > 1 && (
                                        <>
                                            <button
                                                onClick={prevImage}
                                                className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow-md z-20 border border-steel-100"
                                            >
                                                <svg className="w-5 h-5 text-steel-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                                                </svg>
                                            </button>
                                            <button
                                                onClick={nextImage}
                                                className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow-md z-20 border border-steel-100"
                                            >
                                                <svg className="w-5 h-5 text-steel-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div className="px-4 py-6 md:px-0">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                                ...
                                {(product.is_best_selling || product.badge === 'Best Selling') && (
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded text-[10px] font-bold tracking-wider bg-gradient-to-r from-violet-600 to-indigo-600 text-white uppercase shadow-sm shadow-indigo-500/20">
